...
label = Label(taz, text="Hotel Management System", fg="red", bg="yellow", font=("comic sans Ms", 30, "bold"), padx=380,
              pady=0)
...
def billWindow():
    ...
    printButton = Button(taz, text="Print Bill", font=("ariel", 10, "bold"), width=20, fg="green", bd=10,
                         command=printbill)
...
def displaybill():
    ...
    # print(data)
    for row in data:
        tazTV1.insert('', 'end', text=row['datetime'],
                      values=(row["costomer_name"], row['contact_no'], row['item_name'], row['cost']))
    conn.close()
